...
if ($character === "mage") {
    if (($birth_year % 4 != 0) || (($birth_year % 100 == 0) && ($birth_year % 400 != 0))) {
        $validation_error .= "Mages must be born in a leap year. <br>";
    }
}

if (ctype_upper(substr($name, 0, 1))) {
    $validation_error .= "Names must start with a lowercase letter. <br>";
}

if (substr($email, -4) !== ".com") {
    $validation_error .= "Emails must end in .com. <br>";
}
